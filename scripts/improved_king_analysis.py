#!/usr/bin/env python3
"""
Improved King's Method Reverse Engineering Script
=================================================
...
def extract_sku_department_combinations(test_data):
    ...
    # Show unique SKUs and departments
    print(f"\nUnique SKUs: {combinations['Oracle Item Number'].nunique()}")
    print(f"Unique Departments: {combinations['Department Name'].nunique()}")
...
def analyze_demand_patterns(demand_data, combinations):
    ...
    # Show unique SKUs and departments in demand data
    print(f"Unique SKUs in demand data: {demand_data['Oracle Item Number'].nunique()}")
    print(f"Unique Departments in demand data: {demand_data['Department Name'].nunique()}")
    ...
    for idx, combo in combinations.iterrows():
        ...
        print(f"\nAnalyzing SKU {sku} in {dept} (Dept #{dept_num})...")
        ...
        if len(dept_demand) == 0:
            ...
            # Try matching by department name instead
            dept_demand = demand_data[
                (demand_data['Oracle Item Number'] == sku) & 
                (demand_data['Department Name'] == dept)
            ].copy()
            
            if len(dept_demand) == 0:
                print(f"  No demand data found for SKU {sku} in department {dept}")
                continue
            else:
                print(f"  Found {len(dept_demand)} records by department name")
        else:
            print(f"  Found {len(dept_demand)} records by department number")
    ...
    print(f"\nSuccessfully analyzed {matched_count} out of {len(combinations)} combinations")
    return results
...
def test_kings_formula_variations(analysis_results):
    ...
    for result in analysis_results:
        ...
        print(f"Testing SKU {sku} in {dept}...")
        ...
        for period_name, period_stats in result['period_stats'].items():
            if period_stats['count'] > 1 and period_stats['std'] > 0:
                ...
                print(f"  {period_name}: σD={period_stats['std']:.2f}, predicted={basic_kings:.2f}, actual={actual_ss}")
    ...
    print(f"\nGenerated {len(formula_results)} formula test results")
...
def analyze_formula_accuracy(formula_results):
    ...
    if len(df) == 0:
        print("No formula results to analyze!")
        return None, df
...
def find_exact_formula(df, accuracy_metrics):
    ...
    if accuracy_metrics is None or len(accuracy_metrics) == 0:
        print("No accuracy metrics available!")
        return None, None
    
    # Look for formulas with very high accuracy
    high_accuracy = accuracy_metrics[accuracy_metrics['MAPE'] < 10]  # Less than 10% error
    
    if len(high_accuracy) > 0:
        print(f"Found {len(high_accuracy)} formulas with <10% error:")
        ...
        print("No formulas found with <10% error. Looking at top performers...")
...
def main():
    ...
    print("🔍 Improved King's Method Reverse Engineering Analysis")
    print("=" * 60)
    ...
    if accuracy_metrics is not None:
        ...
        # Save results
        df.to_csv('scripts/improved_king_analysis_results.csv', index=False)
        accuracy_metrics.to_csv('scripts/improved_king_accuracy_metrics.csv')
        
        print(f"\nResults saved to:")
        print(f"  - scripts/improved_king_analysis_results.csv")
        print(f"  - scripts/improved_king_accuracy_metrics.csv")
    else:
        print("\n❌ No valid results generated. Check data matching issues.")
