"""
CedarSim Dashboard API with Real Data Integration

Flask API server for the CedarSim dashboard frontend using real 5,941 SKU dataset.
...
def initialize_antology():
    """Initialize the AntologyGenerator with real data from CSV files."""
    ...
    try:
        ...
        antology, data_integrator = create_integrated_antology(use_validation_subset=False)  # Use full dataset
        
        # Create frontend generator
        frontend_generator = FrontendDataGenerator(antology)
        
        logger.info("AntologyGenerator initialized successfully with real data")
        logger.info(f"Loaded {len(antology.sku_registry)} unique SKUs across {len(antology.locations)} locations")
...
if __name__ == '__main__':
    try:
        ...
        print("Initializing AntologyGenerator with real data...")
        initialize_antology()
